...
const TimeLineCard = ({ listId, title, subTitle, start, end, only, image, imageAlt, children }) => {
	...
	return (
		<div className={`${styles.item} ${left ? styles.leftItem : styles.rightItem} ${isMobile ? styles.itemMobile : ''}`}>
			<img src={image} alt={imageAlt}/>
			...
			{!isMobile && <span className={left ? styles.leftArrow : styles.rightArrow} />}
		</div>
	)
}
